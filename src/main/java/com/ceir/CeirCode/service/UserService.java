...
import com.ceir.CeirCode.repo.app.UserTemporarydetailsRepo;
    ...
    public ResponseEntity<?> getUsertypeData(int type) {
        try {
            ...
            if (type == 0) {
                ...
                usertypeData = usertypeRepo.findBySelfRegisterAndStatus(SelfRegistration.INTERNAL.getCode(), SelfRegistration.Enable.getCode());
                ...
                usertypeData = usertypeRepo.findBySelfRegisterAndStatus(SelfRegistration.EXTERNAL.getCode(), SelfRegistration.Enable.getCode());
                ...
                usertypeData = usertypeRepo.findBySelfRegisterOrSelfRegisterAndStatus(SelfRegistration.EXTERNAL.getCode(), SelfRegistration.OTHER.getCode(), SelfRegistration.Enable.getCode());
        ...
        try {
            ...
            if (rolesOutput > 0) {
                ...
                if (userOutput != null) {
                    ...
                    saveUserTrail(userOutput, "User Management", "Register User", 41, header.getPublicIp(), userDetails.getBrowser());
            ...
            if (output != null) {
                ...
                if (output.getPreviousStatus() == UserStatus.APPROVED.getCode()) {
                    if (otp.getForgotPassword() == 1) {
                        saveUserTrail(output, "User Management", "OTP", 41, otp.getPublicIp(), otp.getBrowser());
                        return validateNewUser(output, otp);
                    } else {
                        saveUserTrail(output, "User Management", "OTP", 41, otp.getPublicIp(), otp.getBrowser());
    ...
    public ResponseEntity<?> changeUserStatus(ChangeUserStatus userStatus) {
        ...
        if (user != null) {
            if (userStatus.getAction() == 0) {
                ...
                if (userData != null) {
                    saveUserTrail(userData, "Registration Request", "UserStatus", 41, userStatus.getPublicIp(), userStatus.getBrowser());
                }
                user.setPreviousStatus(user.getCurrentStatus());
                ...
                if (usertypeList.contains(userStatus.getUsertype())) {
                    ...
                    if (userData != null) {
                        saveUserTrail(userData, "Registration Request", "AddedRole", 8, userStatus.getPublicIp(), userStatus.getBrowser());
                    }
                ...
                usertypeList.add(6l);
                if (userData != null) {
                    saveUserTrail(userData, "Registration Request", "DeletedRole", 8, userStatus.getPublicIp(), userStatus.getBrowser());
                }
                user.setRemark(userStatus.getRemark());
                user.setReferenceId(userStatus.getReferenceId());
                user.setModifiedBy(userData.getUsername());
    ...
    public ResponseEntity<?> updateProfile(UserProfile userProfile) {
        try {
            ...
            if (userProfile.getPassword().equals(userProfileData.getUser().getPassword())) {
                ...
                saveUserTrail(userData, "User Management", "Updated", 41, userProfile.getPublicIp(), userProfile.getBrowser());
